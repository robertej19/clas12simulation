...
def SCard_Entry(DBname,UID,timestamp,scard_dict,filename):
    ...
    with open(filename, 'r') as file: scard = file.read()
    print(scard)
    strn = "UPDATE Scards SET {0} = '{1}' WHERE timestamp = {2};".format('scard',scard,unixtimestamp)
    utils.sql3_exec(file_struct.DBname,strn)
...
SCard_Entry(file_struct.DBname,UID,unixtimestamp,scard_fields.data,filename)
